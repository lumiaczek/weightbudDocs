...
Tutaj użytkownik może wpisać swoje dane aby później je aktualizować i sprawdzić swoje postępy. Dzięki temu będzie można również sprawdzić BMI, CPM itp. Mamy tutaj zaimplementowaną podstawową obsługę błędów przed wysłaniem pustego pola.
	...
	<gridLayout columns="45, *" rows="45, *" class="w-full h-screen bg-main pt-3 px-2">
		...
		>
			...
			<stackLayout class="mt-3">
				...
				/>
				{#if errors.nameError}
					<label text="Proszę wprowadzić imię" class="mt-1 text-red-500 mx-2" />
				{/if}
			</stackLayout>
			<stackLayout>
				...
				{#if errors.heightError}
					<label text="Proszę wprowadzić wzrost" class="mt-1 text-red-500 mx-2" />
				{/if}
			</stackLayout>
			<stackLayout>
				<flexBoxLayout flexDirection="row" justifyContent="space-between" class="mx-2 mt-2">
				...
				{#if errors.weightError}
					<label text="Proszę wprowadzić wagę" class="mt-1 text-red-500 mx-2" />
				{/if}
			</stackLayout>
			<button
...
![UserConfig](../public/Screenshot_20221129_203954.jpg)
